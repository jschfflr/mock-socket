...
test('that url query parameters are ignored', t => {
  ...
  const connection = networkBridge.get('wss://not-real/');
...
test('that network bridge has no connections by default', t => {
  t.deepEqual(networkBridge.availableServers.size, 0, 'Url map is empty by default');
});
...
test('that network bridge has no connections by default', t => {
  ...
  t.deepEqual(networkBridge.availableServers.size, 0, 'nothing was added to the url map');
});

test('that attachServer adds a server to url map', t => {
  const result = networkBridge.attachServer(fakeObject, 'ws://localhost:8080');
  const connection = networkBridge.get('ws://localhost:8080');
...
test('that removing server and websockets works correctly', t => {
  ...
  t.deepEqual(networkBridge.availableServers.size, 0, 'Url map is back in its default state');
});
